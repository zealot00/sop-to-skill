...
JSON
RUN_RESP="$(mktemp)"
RUN_CODE="$(curl -sS -o "$RUN_RESP" -w "%{http_code}" -X POST "${BASE_URL}/v1/runs" -H 'Content-Type: application/json' --data-binary @/tmp/req-run.json || true)"
...
JSON
TEST_RESP="$(mktemp)"
TEST_CODE="$(curl -sS -o "$TEST_RESP" -w "%{http_code}" -X POST "${BASE_URL}/v1/tests/runs" -H 'Content-Type: application/json' --data-binary @/tmp/req-test-run.json || true)"
