"""Claude Tools JSON generator.

Generates Claude/MCP tools.json from SOP rules.
"""
from typing import Dict, Any, List
from scripts.models import ExtractedRules, Constraint, ConstraintLevel, DecisionTable, DecisionRule


class ClaudeToolsGenerator:
    """Generator for Claude Tools JSON format."""

    def generate(self, rules: ExtractedRules, skill_name: str) -> List[Dict[str, Any]]:
        """Generate Claude Tools JSON.
        ...
        Returns:
            List of Claude tool dictionaries.
        """
        tools = []
    ...
    def _constraint_to_tool(self, constraint: Constraint) -> Dict[str, Any]:
        """Convert constraint to Claude tool.

        Args:
            constraint: Constraint rule.

        Returns:
            Claude tool dictionary.
        """
    ...
    def _decision_to_tool(self, decision: DecisionTable) -> Dict[str, Any]:
        """Convert decision table to Claude tool.
    ...
    def _generate_info_tool(self, skill_name: str, rules: ExtractedRules) -> Dict[str, Any]:
        ...
        return {
            "name": "get_skill_info",
            "description": f"获取 {skill_name} 的帮助信息和规则说明，包含 {constraint_count} 个约束规则和 {decision_count} 个决策表",
            "input_schema": {
                "type": "object",
                "properties": {
                    "topic": {
